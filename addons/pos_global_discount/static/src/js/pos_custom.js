openerp.pos_global_discount = function (instance) {
    ...
    instance.point_of_sale.PaymentScreenWidget = instance.point_of_sale.PaymentScreenWidget.extend({
        ...
        get_discount_on_order:function(){
            ...
            if (use_dict(order.getName()) == undefined){
                return 0.00
            }
            else{
                return use_dict(order.getName());
            }
        },
        show: function(){
            this._super();
...
//                });
                ...
                buttons: [{
                        text: _t("Apply"), click: function(){
                            ...
                            if(selectedOrder.getTotalTaxIncluded() < inp_val){
                                ...
                                disc_check1 = true;
                                disc_check = true;
                                use_dict[selectedOrder.getName()] = parseFloat(inp_val);
        ...
        update_discount_summary: function() {
            selectedOrder = pos.get('selectedOrder');
            if (use_dict[selectedOrder.getName()] != undefined){
                disc = use_dict[selectedOrder.getName()];
            }else{
                disc = 0.00;
            }
            
            this.$('.payment-fixed-discount').html(this.format_currency(disc));
        },
...
//            disc_check = false;
//            disc_check1 = false;
            delete use_dict[currentOrder.getName()];
        },
    ...
    instance.point_of_sale.Order = instance.point_of_sale.Order.extend({
        get_discount_on_order:function(){
            var currentOrder = this.pos.get('selectedOrder');
            if(use_dict[currentOrder.getName()] != undefined){
                return use_dict[currentOrder.getName()];
            }
            else{
                return 0.00;
            }
        ...
        getTotalTaxIncluded: function() {
            ...
            var currentOrder = this.pos.get('selectedOrder');
            if(use_dict[currentOrder.getName()] != undefined){
                disc =  use_dict[currentOrder.getName()];
            }
            if (disc_check === true){
        ...
        export_as_JSON: function() {
            ...
            if (disc_check1 == true)
            {
                var disc_amount = this.get_discount_on_order();
                var check = false;
                if(disc_amount > 0){
                    check = true;
                }else{
    ...
    instance.point_of_sale.OrderWidget = instance.point_of_sale.OrderWidget.extend({
        ...
        update_summary: function(){
            ...
            if (use_dict[order.getName()] != undefined){
                disc = use_dict[order.getName()];
                var taxes     = order ? total - order.getTotalTaxExcluded() + disc : 0;
            }else{
